...
jQuery( document ).ready(function() {
	jQuery( ".awis-block" ).first().addClass( "awis-block-first" );
	if(jQuery("body").hasClass("cms-accueil")){
		
		var elementOffset  = jQuery(".awis-block-first").offset().top;
		var newMarginTop = elementOffset - 40;
		var newMarginTopStr = "-" +newMarginTop + "px";
		jQuery(".awis-block-first").css({"margin-top":newMarginTopStr});
	}
	else{
		jQuery(".awis-block-first").css({"display":"none"});
	}
});
