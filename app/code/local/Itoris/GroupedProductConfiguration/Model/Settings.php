...
class Itoris_GroupedProductConfiguration_Model_Settings extends Varien_Object {
	...
	private $tableView = 'itoris_groupedproduct_rules_view';
	private $tableSettings = 'itoris_groupedproduct_rules_settings';
	private $tableTextSettings = 'itoris_groupedproduct_rules_settings_text';

	private $textOptions = array();

	private $checkboxOptions = array();

    const SHOW_QTY_INPUT = 0;
    const SHOW_QTY_CHECK = 1;
	...
	public function __call($method, $args) {
		if (substr($method, 0, 3) == 'get') {
			...
			if (!$this->isLoaded && isset($this->settings['by_default'][$key])) {
	...
	public function isParentValue($key, $checkWebsite = false) {
		...
		if ($checkWebsite) {
			if (isset($this->settings['website'][$key])) {
				return false;
			}
		}
		return true;
