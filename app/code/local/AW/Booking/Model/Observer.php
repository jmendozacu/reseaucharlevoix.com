...
class AW_Booking_Model_Observer {
    ...
    public function bindCartItems($event) {
        ...
        if( ($Product->getAwBookingRangeType()!=='date_fromto') && ($From->compare($To))>=0)
    ...
    public function bindOrderItems($observer) {
        ...
        foreach ($items as $item) {
            ...
            $From = new Zend_Date($_date_from, 'YY-MM-dd');//Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT));
            $To = new Zend_Date($_date_to, 'YY-MM-dd');//Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT));
