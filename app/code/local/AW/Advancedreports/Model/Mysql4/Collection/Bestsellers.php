        ...
        if ($skuType == AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_SIMPLE) {
            $this->getSelect()
                ...
                ->joinLeft(
                    ...
                    "(item.parent_item_id = item2.item_id AND item2.product_type IN ('configurable'))", array()
                )
            ;
        } else {
            $filterField = Mage::helper('advancedreports')->confOrderDateFilter();
            ...
            $this->getSelect()
                ->join(
                    ...
                    array(
                        ...
                        'sum_total' => "(
                            ...
                            - COALESCE(SUM(t_discount.item_discount),0))* $currencyRate
                        )",
                        'name' => 'name',
                        'sku' => 'realP.sku',
                    )
                ...
                ->joinLeft(
                    array('t_discount' => new Zend_Db_Expr(
                        ...
                        LEFT JOIN {$itemTable} AS `t_item2` ON (t_item2.parent_item_id IS NOT NULL AND t_item2.parent_item_id = t_item.item_id AND t_item.product_type IN ('configurable', 'bundle'))
        ...
        $this->getSelect()
            ->joinLeft(
                array('profit' => new Zend_Db_Expr(
                    ...
                    LEFT JOIN {$itemTable} AS `item2` ON item2.item_id = item.parent_item_id AND item2.product_type = 'configurable, bundle'
