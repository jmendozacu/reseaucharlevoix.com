        ...
        $this->getSelect()
            ->joinLeft(
                new Zend_Db_Expr(
                    ...
                    LEFT JOIN {$itemTable} AS `t_item2` ON t_item.parent_item_id IS NOT NULL AND t_item.parent_item_id = t_item2.item_id AND t_item2.product_type = 'configurable'
                    WHERE (order.{$filterField} >= '{$dateFrom}' AND order.{$filterField} <= '{$dateTo}')
                    GROUP BY t_item.product_id)"
                ),
                "item.product_id = t.item_product_id ",
                array(
                    'estimate_qty'           => "COALESCE(t.sum_qty, 0)",
                    'estimate_days'          => "COALESCE(t.sum_qty, 0)/{$days}",
                )
            );
    }
}