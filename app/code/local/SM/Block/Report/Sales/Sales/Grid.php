...
//                ->setPeriod($filterData->getData('period_type'))
                ...
                //TODO: sales_flat_order SHOULD BE main_table
                ->addFieldToFilter(Mage::getSingleton('core/resource')->getTableName('sales_flat_order').'.till_id',array('eq' => $filterData->getData('till_id')));
        }else{
            $resourceCollection = Mage::getResourceModel($this->getResourceCollectionName())
//                ->setPeriod($filterData->getData('period_type'))
                ->setDateRangeType($dateRange)
        ...
        $resourceCollection->getSelect()->joinLeft(
            array('tran' => $resourceCollection->getTable('xpos/transaction')),
            'tran.order_id = '.Mage::getSingleton('core/resource')->getTableName('sales_flat_order').'.increment_id',
            array('cash_in', 'cash_out')
        );
...
//                    ->setPeriod($filterData->getData('period_type'))
            ...
            if($filterData->getData('xpos_only') == 1){
                $totalsCollection ->addFieldToFilter('xpos',array('eq' => 1));
            }

            $totalsCollection->getSelect()->joinLeft(
                array('tran' => $totalsCollection->getTable('xpos/transaction')),
                'tran.order_id = '.Mage::getSingleton('core/resource')->getTableName('sales_flat_order').'.increment_id',
                array('cash_in' => 'SUM(cash_in)', 'cash_out')
            );
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('subtotal', array(
            ...
            'index'         => 'subtotal',
            'renderer' => 'xpos/adminhtml_report_grid_renderer_currency',
            'rate'          => $rate,
            'total'          =>  'sum',
        ));
