 ...
 */
class VDesign_BookmePro_Block_Adminhtml_Adventures_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	public function __construct()
	{
		date_default_timezone_set(Mage::app()->getStore()->getConfig('general/locale/timezone'));
		parent::__construct();
		$this->setId('adminhtml_adventures_grid');
		$this->setDefaultSort('adventure_id');
	...
	protected function _prepareCollection()
	{
		...
		->getSelect()
 		->where("main_table.book_type = 'Adventure' AND main_table.date_from >= '$today'");
	...
	protected function _prepareColumns()
	{
		...
		$this->addColumn('date_from', array(
				'header' => $helper->__('Starting day'),
				'type' => 'date',
				'index'  => 'date_from'
		));
		$this->addColumn('time_from', array(
				'header' => $helper->__('Starting time'),
				'type'  => 'options',
				'index'  => 'time_from',
				'options' => Mage::getSingleton('bookmepro/item_options')->getAdvStartTimes()
		));
		$this->addColumn('date_to', array(
				'header' => $helper->__('Ending day'),
				'type' => 'date',
				'index'  => 'date_to'
		));
		$this->addColumn('time_to', array(
				'header' => $helper->__('Ending time'),
				'type'  => 'options',
				'index'  => 'time_to',
				'options' => Mage::getSingleton('bookmepro/item_options')->getAdvEndTimes()
		));
		...
		$this->addExportType('*/adminhtml_adventures/exportCsv', $helper->__('CSV'));
		$this->addExportType('*/adminhtml_adventures/exportExcel', $helper->__('Excel XML'));
		return parent::_prepareColumns();
	}
	
	public function getRowUrl($row)
	{
		return $this->getUrl('*/adminhtml_adventure/index', array('session_id' => $row->getSessionId()));
	}
	...
	protected function _addColumnFilterToCollection($column)
	{
		if($this->getCollection() && $column->getFilter()->getValue())
		{
			...
			if($column->getId() == 'p_name'){
				$this->getCollection()->addFieldToFilter('ea.value', array('like' => "%$value%"));
				return $this;
			}
		}
