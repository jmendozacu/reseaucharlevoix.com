...
class VDesign_Bookme_Helper_Availible extends Mage_Core_Helper_Abstract{
	...
	public function addRefreshingItems($product, $date){
		$time = strtotime($date);
		$refreshDays = $product->getData('refresh_days');
		$days = array();
		for($d = 1; $d <= $refreshDays; $d++)
		{
			if($this->isInExcluded($product, date('Y-m-d', $time - ($d * 60 * 60 * 24)))){
				if($this->refreshInExcluded($product, date('Y-m-d', $time - ($d * 60 * 60 * 24)))){
					$_d = date('Y-m-d', $time - ($d * 60 * 60 * 24));
					$days[] = $_d;
				}else{
					$_d = date('Y-m-d', $time - ($d * 60 * 60 * 24));
					$days[] = $_d;
		...
		if(count($days) > 0)
		{
			...
			->addFieldToFilter('booked_to', array('in' => $days));
	...
	public function addRefreshDay($product, $time){
		$out = 0;
		if($product->getAttributeText('billable_period') == 'Day')
		{
			$refreshDays = $product->getData('refresh_days');
	...
	public function refreshInExcluded($product, $date){
			$exludeDays = $product->getData('exclude_day');
			foreach ($exludeDays as $exday){
				if($exday['period_type'] == 1){
					$exfrom = $exday['from_date'];
					$exTo = $exday['to_date'];
					if(strtotime($exfrom) <= strtotime($date) && strtotime($exTo) >= strtotime($date) && $exday['refresh_day'])
						return true;
					else continue;
				}
				...
				if($exday['period_type'] == 2){
					$exday = strtotime($exday['value']);
					$exday = date('Y-m-d', $exday);
					if(strtotime($date) == strtotime($exday) && $exday['refresh_day'])
						return true;
					else continue;
				}
				...
				if($exday['period_type'] == 3){
					$dw = date( "w", strtotime($date));
					if($exday['value'] == $dw && $exday['refresh_day']){
						return true;
					}
					else continue;
				}
				...
				if($exday['period_type'] == 4){
					$day = explode("-", date('Y-m-d', strtotime($date)));
					if($day[2] == $exday['value'] && $exday['refresh_day'])
						return true;
					else continue;
				}
			}
			...
			*/
			foreach ($sessions as $session){
				if($session['session_day'] == 11 && isset($session['spec_day'])){
					if($helper->formatDate($session['spec_day'], Varien_Date::DATE_INTERNAL_FORMAT) ==
							 $helper->formatDate(date('Y-m-d H:i:s', $date / 1000), Varien_Date::DATE_INTERNAL_FORMAT)){
						$output = '';
						foreach ($session['sessions'] as $time){
							$session_timestamp = (($time['hour'] * 60 * 60) + ($time['minute'] * 60));
							$output .= $session_timestamp.'#'.$this->isSessionAvailible($product, $date + ($session_timestamp * 1000)).',';
