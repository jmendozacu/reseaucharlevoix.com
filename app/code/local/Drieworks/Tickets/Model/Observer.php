			...
			$themebg = $ticket['themebg'];
				...
				switch ($ticket_type) {	
					...
					case 'Ticket': //ticket
							...
							$ticket_type = 'ticket';
							Mage::log("INSERT INTO `ticket_orders` (`ticket_id`,`ticket_type`,`gender`, `name`, `email`,`birthday`,`adress`,`zipcode`,`city`,`themecolor`,`themebg`,`product`,`price`,`date`,`order_id`) VALUES (NULL,'" . $ticket_type . "','" . $gender . "','" . $naam . "','" . $email . "','" . $verjaar . "','" . $adress . "','" . $zipcode . "','" . $city . "','" . $themecolor . "','" . $themebg . "','" . $kaarten . "','" . $PriceTax . "',NOW(),'" . $RealOrder . "')");
							...
							Mage::log('$availableFor: ' . $availableFor);
							if($availableFor == 'Adults' || $availableFor == 'Adult' ) { // for adults
								$write->query("UPDATE `ticket_barcode` SET `order_id` = " . $RealOrder . ",`ticket_id` = " . $lastinsertId . " WHERE `order_id` = 0 AND `invoice_id` = '' AND `available_for` = 'A' LIMIT 1");
							} elseif($availableFor == 'Children' || $availableFor == 'Child' ) { //for children
								Mage::log("UPDATE `ticket_barcode` SET `order_id` = " . $RealOrder . ",`ticket_id` = " . $lastinsertId . " WHERE `order_id` = 0 AND `invoice_id` = '' AND `available_for` = 'C' LIMIT 1");
								$write->query("UPDATE `ticket_barcode` SET `order_id` = " . $RealOrder . ",`ticket_id` = " . $lastinsertId . " WHERE `order_id` = 0 AND `invoice_id` = '' AND `available_for` = 'C' LIMIT 1");
							} else { // For elderly 65+
								$write->query("UPDATE `ticket_barcode` SET `order_id` = " . $RealOrder . ",`ticket_id` = " . $lastinsertId . " WHERE `order_id` = 0 AND `invoice_id` = '' AND `available_for` = 'E' LIMIT 1");
							}
							break;
