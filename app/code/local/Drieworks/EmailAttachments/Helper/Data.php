<?php
class Drieworks_EmailAttachments_Helper_Data extends Mage_Core_Helper_Abstract {
    
    public function addAttachment($pdf, $mailObj, $name = "order.pdf") {
        try{
            $file = $pdf->render();
            $mailObj->getMail()->createAttachment($file,'application/pdf',Zend_Mime::DISPOSITION_ATTACHMENT,Zend_Mime::ENCODING_BASE64,$name.'_et_Billets.pdf');
        } catch (Exception $e){
