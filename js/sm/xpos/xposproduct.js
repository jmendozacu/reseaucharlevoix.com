...
(function (jQuery) {
    jQuery(document).ready(function () {
        ...
        jQuery('#search-box').keyup(function (event) {
            ...
            if (keyCode == window.KeyCode.UP || keyCode == window.KeyCode.DOWN || keyCode == window.KeyCode.LEFT || keyCode == window.KeyCode.RIGHT || keyCode == window.KeyCode.SOFT_BACKSLASH || keyCode == window.KeyCode.SPACE) {
            ...
            delay(function () {
                ...
                if (offlineSearch && term != "") {
                    ...
                    window.ITEM_NUMBER_IN_PRODUCT_POOL = result.length;
                    window.flagAddedProduct = false;
                    if (window.ITEM_NUMBER_IN_PRODUCT_POOL == 1 && (window.BARCODE_MODE|| !!lucky_search)) {
                        addToCart(result[0].id);
                        setTimeout(function() {jQuery('#search-box').select();}, 100);
                        window.flagAddedProduct = true;
                    }
                    ...
                    if(window.flagAddedProduct){
                        jQuery('#search-box').focus().select();
                    }
                } else {
                    if (isOnline() && term != "") {
                        iLog('Online Search');
                        ...
                        jQuery.getJSON(url, function (json) {
                            ...
                            window.ITEM_NUMBER_IN_PRODUCT_POOL = data.length;

                            if (window.ITEM_NUMBER_IN_PRODUCT_POOL == 1 && window.BARCODE_MODE) {
                                addToCart(data[0].id);
                                setTimeout(function() {jQuery('#search-box').select();}, 100);
                            }
...
var noDiscountTemplate =
    ...
    + '<a href="#" class="remove" onclick="removeFromCat(\'{id}\')"></a><h5 class="hover1 {config_change}-{id}" data-child-product-id={child_product_id}>{name}</h5>'
    + '<h6 class="sku-text">{sku}</h6>'
    + '{option}</div></td><td class="qty align-center"><input id="item-qty-{id}" maxlength="12" value="{qty}" data-item-base-id="{baseId}" data-item-id="{id}" class="checkout-item-qty input-text item-qty {is_qty_decimal}" name="item[{id}][qty]" onchange="changeQty(\'{id}\')"></td><td class="tax align-center">{tax}%</td><td class="price align-right"><span class="no-change">{price}</span></td><td class="subtotal align-right"><div class="subtotal-list">{subtotal}<span></span></div></td></tr>';
/**
 * Author here?
 *
 * Add to visual
