...
AdminOrder.prototype = {
    ...
    loadTotalQuote: function (method, indicator, params) {
        ...
        //        iLog('Change Shipping Method. Set Total', response.data);
        //        jQuery('#grandtotal').text(addCommas(response.data.toFixed(2)));
        //    }.bind(this)
        //});
        jQuery.ajax({
            url: urlTotal,
    ...
    printZreport: function (report_type, transfer_amount, diff_total) {
        ...
        var cash_count = parseFloat(jQuery('#xpayment_cashpayment').val());
        var split_count = parseFloat(jQuery('#xpaymentMultiple').val());
        ...
        if (isNumber(split_count))
            var post_split_count = '/split_count/' + split_count;
        else var post_split_count = '';
        ...
        var win = window.open(print_url + post_cashier + post_reporttype +
            post_transfer + post_diff + post_cash_count + post_split_count + post_check_count + post_cc_count + post_other_count, '',
            'width=900,height=600,resizable=1,scrollbars=1');
        win.focus();
    },
...
function canUseMaxAmout(method) {
    ...
    var currentTotal = 0;
    var flag = false;
    var currentEleId = method + '_input_price';
    if (parseFloat(jQuery('#' + currentEleId).val()) >= grandTotal) {
        maxAmount.setData(grandTotal.toFixed(2), 3);
        return maxAmount;
    }

    jQuery('.amountSplit').each(function (index) {
        if (jQuery(this).attr('id') == currentEleId) return true;
        if (parseFloat(jQuery(this).val()) < grandTotal && parseFloat(jQuery(this).val()) > 0) {
            flag = true;
        }
        currentTotal += parseFloat(jQuery(this).val());
    });
    if (currentTotal >= grandTotal || currentTotal == 0) {
        //if(currentTotal == grandTotal && flag == true){
        //    iLog('when call calculator max amount == GT and Flat = true  =>still Split payment');
        //    maxAmount.setData(0, 2);
        //    return maxAmount
        //}
        iLog('when call calculator max amount 0 == currentTotal >= GrandTotal. Will set Input and price =0 and use Single Payment', null, 5);
        jQuery('.amountSplit').each(function (index) {
            jQuery(this).val(0);
        });
        jQuery('.splitPrice').each(function (index) {
            jQuery(this).html('');
        });
        maxAmount.setData(grandTotal.toFixed(2), 0);
        return maxAmount;
    } else {
        maxAmount.setData((grandTotal - currentTotal).toFixed(2), 1);
        return maxAmount;
    }
}
...
function setPriceSplitPayment(method) {
    ...
    if (grandTotal >= currentInputAmount && currentInputAmount > 0) {
        ...
        jQuery(methodPrice).removeAttr('disabled').html('  -  Split: ' + currentInputAmount);
    }
...
window.splitPaymentCheck = {
    canCheckOut: function () {
        ...
        return !(window.canCheckOutWithSplitPayment == false && jQuery('#is_split_payment_method_hidden').val() == '1');
    },
